...
void inicializarPartida(t_partida *partida) {
    ...
    partida->conx.conexion = (t_conexion *)malloc(sizeof(t_conexion) * partida->conx.num_conexiones);

    // Conexión: Torreón -> Armería
    strcpy(partida->conx.conexion[0].id_cnx, "CX01");
    partida->conx.conexion[0].id_org = 1;
    partida->conx.conexion[0].id_dst = 2;
    partida->conx.conexion[0].estado = 1; // Abierta

    // Conexión: Armería -> Trono
    strcpy(partida->conx.conexion[1].id_cnx, "CX02");
    partida->conx.conexion[1].id_org = 2;
    partida->conx.conexion[1].id_dst = 3;
    partida->conx.conexion[1].estado = 0; // Bloqueada (Requiere llave)
